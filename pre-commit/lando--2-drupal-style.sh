...
# - [Lando tooling for lint-style-php](https://github.com/mparker17/drupal-lando_snippets/blob/master/tooling/lint-style-php.yml)
...
function test_file {
    ...
    if [ -x "$(command -v lando)" ] ; then
        ...
        set +e
        lando lint-style-php "$file"
        set -e
    else
        echo "Can't run Drupal style linter because the lando executable isn't on the PATH."
    fi
}
